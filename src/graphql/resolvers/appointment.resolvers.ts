...
export const getAppointments = async (_: unknown, __: unknown, context: JwtContext): Promise<Appointment[]> => {
...
export const getAppointment = async (_: unknown, { id }: ID, context: JwtContext): Promise<Appointment> => {
  const { payload } = await verifyJWT(context)
  checkRoles(payload, 'ADMIN')
